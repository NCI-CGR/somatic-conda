...
echo "Running somatic_conda_wrapper_ccad.sh ...."
...
clusterMode='"'"qsub -v PATH -v conf -v TMPDIR -cwd -q all.q -pe by_node {threads} -l mem_free=32G  -o ${LOG_SNAKE}/ -e ${LOG_SNAKE}"'"'
...
cmd="conf=$configFile snakemake --cores $numJobs  --verbose --use-conda -p -s ${execDir}/workflow/Snakefile.conda_docker --rerun-incomplete --cluster ${clusterMode} --jobs $numJobs --conda-frontend conda --latency-wait ${latency} --use-singularity --default-resources 'mem_mb=10000' 'tmpdir=\"/mnt/nfs/gigantor/ifs/DCEG/Projects/CoherentLogic/somatic/workspace7/TMP\"' &> ${LOG_DIR}/run_Snakefile_$(date +%y%m%d).out"
