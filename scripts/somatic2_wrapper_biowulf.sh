...
clusterMode = "sbatch -p norm -o ${LOG_SNAKE}/slurm-%j.out -e ${LOG_SNAKE}/slurm-j%j.err -t 200:00:00 "
...
cmd="conf=$configFile snakemake --cores $numJobs --verbose --use-conda  -p -s ${execDir}/workflow/Snakefile --rerun-incomplete --cluster \"${clusterMode}\" --jobs $numJobs --conda-frontend mamba --latency-wait ${latency} &> ${LOG_DIR}/run_Snakefile_$(date +%y%m%d).out"
